...
```
```{r}
# check Conduent case - number of firms decreased 21 to 8
final_data_16_21 %>% filter(str_detect(actuarial_firm_name, "Conduent")) %>% filter(fy == 2016) %>% select(PlanName) %>% arrange(PlanName)
```
