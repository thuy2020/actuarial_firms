...
reason_data_clean_16_21 %>% 
  ...
  mutate(actuarial_firm_name = str_to_title(actuarial_firm_name)) %>% select(actuarial_firm_name) %>% distinct() %>% arrange(actuarial_firm_name)
...
actuarial_firm_data_16_21 <- reason_data_clean_16_21 %>% 
  ...
  mutate(actuarial_firm_name = case_when(str_detect(actuarial_firm_name, "(?i)bolton") ~
                                           "Bolton",
    ...
    #"Office Of The State Actuary" ~ "Office Of The State Actuary - Washington" ??? ~ "The Office Of The State Actuary"
...
final_data_16_21  <- ppd_data_clean %>% 
  ...
  # recheck names - still see some similar names here --> check with Truong/ Zach
